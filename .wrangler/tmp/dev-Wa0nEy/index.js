...
// .wrangler/tmp/bundle-jyuPtz/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-jyuPtz/middleware-insertion-facade.js
init_modules_watch_stub();
...
function formatAnswerValue(field, value) {
  if (field.field_key === "items" && typeof value === "string") {
    try {
      ...
      if (Array.isArray(
        ...
        const lines = parsed.filter(
          ...
          (item, index) => {
            const row = item;
            const name = typeof row.item === "string" ? row.item.trim() : "";
            if (!name) {
              return "";
            }
            ...
            return [
              `${index + 1}. ${name}`,
              quantity || unit ? `Qty: ${[quantity, unit].filter(Boolean).join(" ")}` : "",
              price ? `Price: ${price}` : "",
              details
            ].filter(
              Boolean
...
function getRequestSummary(detailsJson) {
  if (!detailsJson) {
    return "";
  }
  try {
...
__name(getRequestSummary, "getRequestSummary");
...
async function requireChatAdmin(request, env) {
  const session = await getAdminSession(
    request,
    env
  );
...
__name(requireChatAdmin, "requireChatAdmin");
async function listAdminThreads(request, env) {
  const auth = await requireChatAdmin(
    request,
    env
  );
  if (auth) {
...
__name(listAdminThreads, "listAdminThreads");
async function getAdminThread(request, env, url) {
  const auth = await requireChatAdmin(
    request,
    env
  );
  ...
  if (!thread) {
    return jsonResponse12(
      {
        error: "This request does not have a chat thread yet."
      },
      404
    );
  }
  ...
  return jsonResponse12({
    ...
    messages: messages.results.map(
      (message) => ({
        ...
        message: message.message,
        createdAt: message.created_at,
        mine: message.sender_type === "admin" || message.sender_type === "support"
      })
    )
  });
}
__name(getAdminThread, "getAdminThread");
async function postAdminMessage(request, env) {
  const auth = await requireChatAdmin(
    request,
    env
  );
  ...
  const requestNumber = typeof body.requestNumber === "string" ? body.requestNumber.trim().toUpperCase() : "";
  const message = typeof body.message === "string" ? body.message.trim() : "";
  if (!requestNumber || !message || message.length > MAX_MESSAGE_LENGTH) {
    return jsonResponse12(
      {
        error: `Request and a 1-${MAX_MESSAGE_LENGTH} character message are required.`
      },
      400
    );
  }
...
__name(postAdminMessage, "postAdminMessage");
async function handleChatRoute(request, env, url) {
  ...
  if (request.method === "GET" && url.pathname === "/api/chat/admin/threads") {
    return listAdminThreads(
      request,
      env
    );
  }
  if (request.method === "GET" && url.pathname === "/api/chat/admin/thread") {
  ...
  if (request.method === "POST" && url.pathname === "/api/chat/admin/messages") {
