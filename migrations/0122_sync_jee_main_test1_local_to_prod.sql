-- 0122_sync_jee_main_test1_local_to_prod.sql
-- Makes LOCAL JEE Main Test 1 match production at repaired orders 22, 25, 71.
-- Run LOCAL only.
--
-- Production already uses:
--   22 -> IN_PROGRAM_JEE_MATH_QT1_REMOTE_PATCH_022
--   25 -> IN_PROGRAM_JEE_MATH_QT1_REMOTE_PATCH_025
--   71 -> IN_PROGRAM_JEE_CHEMISTRY_QT1_REMOTE_PATCH_071

PRAGMA foreign_keys = ON;

-- Remove only the three local mappings that differ from production.
...
AND question_order IN (22,25,71);

-- Reuse the same production patch questions; create them locally if absent.
...
-- Reinsert the exact production mappings.
...
WHERE mt.program_code='JEE'
  ...
  AND mt.exam_level='MAIN'
  AND mt.version=1
  AND mtq.question_order IN (22,25,71)
